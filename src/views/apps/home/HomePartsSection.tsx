...
const PART_OPTIONS = ['Motor', 'Gearbox', 'Wire Rope', 'Hook', 'Panel', 'Brake']

const HomePartsSection = () => {
    const { control, handleSubmit, reset } = useForm({
        defaultValues: { isVisible: true, title: '', subtitle: '', selectedParts: [] as string[] }
    })

    useEffect(() => {
        const savedData = localStorage.getItem('home_parts_data')
        if (savedData) reset(JSON.parse(savedData))
    }, [reset])

    const onSubmit = (data: any) => {
        localStorage.setItem('home_parts_data', JSON.stringify(data))
        alert('Parts Section Saved')
    }
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <FormControl fullWidth>
                                    <InputLabel>Select Parts</InputLabel>
                                    <Select
                                        multiple
                                        {...field}
                                        label='Select Parts'
                                    ...
                                    >
                                        {PART_OPTIONS.map((name) => <MenuItem key={name} value={name}>{name}</MenuItem>)}
                                    </Select>
                                </FormControl>
                            )}
                        />
...
export default HomePartsSection
